...
pagetitle: "Energy-Transitions — Class 3"
...
if (!file.exists(path_csv)) {
  cat("❌ File not found at:", path_csv, "\n")
...
<iframe src="https://drive.google.com/file/d/1A0vraXapL-YH1uKSuyUxCHrF54LiVVX8/preview"
...
ggplot_grammar <- tibble::tibble(
  ...
  `What it does` = c(
    "The dataset you’re visualizing.",
    "How variables map to visual properties such as position, color, or size.",
    "The visual representation — points, bars, lines, boxplots, etc."
  ),
  `Example in FinSESCo context` = c(
    "`finsesco` – our survey of energy efficiency and prosumer behaviour.",
...
**Step 3 — Add color, labels, and a cleaner layout**
...
layers_tbl %>%
  gt() %>%
  tab_header(
    title = md("**Step 4 — Understanding What Each Layer Does**")
  ) %>%
  cols_width(
    Layer ~ px(200),
    Function ~ px(180),
...
**But remember:** these are descriptive patterns — they show associations, not causal effects. Specially in the case for our figure, as the differences between the groups are not so pronounced. In Session 4, we’ll learn to add inference to assess whether these differences are statistically meaningful.
...
```
<iframe src="https://drive.google.com/file/d/1RQ1MWvilTYvr5hxKIOfSN5Mv5e9eij6P/preview"
        width="640" height="360" allow="autoplay"></iframe>
...
1. Define a **threshold** on a scale (e.g., “high” = score ≥ 7 on a 0–10 scale).  
